...
@section('title', 'Dashboard báo cáo nhiệm vụ')

@section('content')

<h2 class="text-center">THỐNG KÊ NHIỆM VỤ</h2>
...
<div class="row">
    ...
    @elseif (auth()->user()->hasRole('sub_admin'))
        ...
        <div class="col-md-3 form-group">
            ...
            <select name="children_agency_id" class="form-control select2">
                <option value="">-- Tất cả đơn vị --</option>

                @foreach ($agencyGroups as $gid => $agencies)
                    @foreach ($agencies as $a)
                        <option value="{{ $a->id }}"
                            @selected(request('children_agency_id') == $a->id)>
                            {{ $a->agency_name }}
                        </option>
                    @endforeach
                @endforeach
    ...
    <div class="col-md-3 form-group">
        ...
        <select name="report_status" class="form-control">
            ...
            <option value="reported" @selected(request('report_status') === 'reported')>
                Đã báo cáo
            </option>
            <option value="not_reported" @selected(request('report_status') === 'not_reported')>
                Chưa báo cáo
            </option>
        </select>
    </div>
    <div class="col-md-3 form-group">
        ...
        <select name="complete_status" class="form-control">
            <option value="">-- Tất cả --</option>
            <option value="completed" @selected(request('complete_status') === 'completed')>
                Đã hoàn thành
            </option>
            <option value="not_completed" @selected(request('complete_status') === 'not_completed')>
                Chưa hoàn thành
            </option>
        </select>
    </div>
    <div class="col-md-3 form-group">
        ...
        <select name="deadline_status" class="form-control">
            <option value="">-- Tất cả --</option>
            <option value="on_time" @selected(request('deadline_status') === 'on_time')>
                Đúng hạn
            </option>
            <option value="overdue" @selected(request('deadline_status') === 'overdue')>
                Trễ hạn
            </option>
        </select>
    </div>
    ...
    <div class="ibox-content">
        <table class="table table-bordered table-striped mission-table">
            ...
            <tr>
                <th>STT</th>
                <th>Nhiệm vụ</th>
                <th>{{ $isAdminOrSupervisor ? 'Cơ quan chủ trì' : 'Phòng ban / Đơn vị' }}</th>
